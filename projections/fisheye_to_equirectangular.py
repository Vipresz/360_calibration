...
def fisheye_to_equirect_calibrated(img, output_width, output_height, lens_calib: LensCalibration,
                                    base_fov=180.0, edge_margin_ratio: float = 0.0):
    """Convert single fisheye to equirectangular with lens calibration.

    The returned ``valid`` mask is True ONLY where the corresponding output
    pixel was sampled from inside the original fisheye disk -- specifically:
      * the source point lies inside the image rectangle,
      * the source point lies inside the lens disk (centered at the lens
        calibration's centre, radius ``min(w, h) // 2``),
      * the source point lies at least ``edge_margin_ratio * radius`` pixels
        AWAY from the disk boundary (so bilinear interpolation on the rim
        cannot leak pixels from outside the disk into the output),
      * the angular distance from boresight is within the lens FOV.

    Args:
        edge_margin_ratio: 0..1 fraction of the lens-disk radius to exclude as
                           an inner safety margin against bilinear-interpolation
                           leakage from outside the disk.  0 = no margin.
    """
    h, w = img.shape[:2]
    ...
    # Validity:
    #   1) Source point inside image rectangle.
    in_rect = (x_fish >= 0) & (x_fish <= w - 1) & (y_fish >= 0) & (y_fish <= h - 1)
    #   2) Source point inside the LENS DISK (centred on the calibrated lens
    #      centre, NOT on the image centre -- otherwise an offset lens leaks
    #      black-bordered pixels into the comparison as fake "valid" zeros).
    dx = x_fish - cx0
    dy = y_fish - cy0
    # Effective disk radius = full radius - safety margin.  The margin keeps
    # bilinear-interpolated pixels at the very rim from sneaking in.
    eff_radius = radius * (1.0 - max(0.0, min(1.0, float(edge_margin_ratio))))
    in_disk = (dx * dx + dy * dy) <= (eff_radius * eff_radius)
    #   3) Within the lens FOV (angular distance bound).
    in_fov = r <= 1.001

    valid = in_rect & in_disk & in_fov
...
def main():
    ...
    parser.add_argument('--calibration', '-c', type=str, help='Calibration file (.toml or .json)')
    ...
    if args.calibration:
        try:
            ...
            for name, lens in [('Lens1', calibration.lens1), ('Lens2', calibration.lens2)]:
                print(f"  {name}: center=({lens.center_x:.4f}, {lens.center_y:.4f}), fov={lens.fov:.1f} deg, k1={lens.k1:.4f}")
                print(f"         yaw={lens.rotation_yaw:.4f} rad, pitch={lens.rotation_pitch:.4f} rad, roll={lens.rotation_roll:.4f} rad")
